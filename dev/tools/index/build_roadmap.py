...
Outputs:
  dev/tools/assets/roadmap_metrics.tsv     full ranked table (leaf readiness: deps, blind)
  dev/tools/assets/roadmap.md              phased roadmap (deliverable)
  dev/tools/assets/symbol_index.json       symbol -> defining file (for dev/tools/draft/scribe_draft.py)

Paths resolve from $PROJECT_HOME (set by environment.sh), or from this script's
location (dev/tools/index/build_roadmap.py -> project root) as a fallback. The MCFM clone is
...
# name -> defining source file) that the DRAFT phase (dev/tools/draft/scribe_draft.py)
...
md.append("Auto-generated by `dev/tools/index/build_roadmap.py` from a Doxygen call graph "
          "(`software/mcfm/doxygen_dep`) fused with static complexity metrics. "
          "Full per-file data: `dev/tools/assets/roadmap_metrics.tsv`.\n")
...
print(f"symbol index: {len(symbols)} symbol(s) -> dev/tools/assets/symbol_index.json")
print("wrote dev/tools/assets/roadmap.md, dev/tools/assets/roadmap_metrics.tsv, dev/tools/assets/symbol_index.json")
